      ...
      <ErpTable headers={['Client', 'Téléphone', 'Ville', 'Source', 'Commandes', 'CA total', '']}>
        {filtered.map(c => (
          ...
          >
            <td style={{ padding: '0.8rem 1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.55rem' }}>
                <div style={{
                  ...
                  fontSize: '0.68rem', color: '#8A7B68', fontWeight: 600, flexShrink: 0,
                }}>
                  {(c.nom || '?').charAt(0).toUpperCase()}
                </div>
                <div>
                  <div style={{ fontSize: '0.82rem', color: '#1A1208', fontWeight: 500 }}>{c.nom}</div>
                  {c.email && <div style={{ fontSize: '0.68rem', color: '#C4B090' }}>{c.email}</div>}
                </div>
              </div>
            </td>
            <td style={{ padding: '0.8rem 1rem' }}>
              <span style={{ fontSize: '0.78rem', color: '#8A7B68', fontFamily: 'monospace' }}>{c.telephone}</span>
            </td>
            <td style={{ padding: '0.8rem 1rem' }}>
              <span style={{ fontSize: '0.78rem', color: '#1A1208' }}>{c.ville ?? '—'}</span>
            </td>
            <td style={{ padding: '0.8rem 1rem' }}>
              {c.source && (
                <span style={{
            ...
            <td style={{ padding: '0.8rem 1rem' }}>
              <span style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: '1rem', color: '#1A1208' }}>{c.totalCommandes ?? 0}</span>
            </td>
            <td style={{ padding: '0.8rem 1rem' }}>
