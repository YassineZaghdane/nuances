...
export default function ErpMarquesPage() {
  ...
  const filtered = useMemo(
    () => marques.filter(m => m.nom.toLowerCase().includes(search.toLowerCase())),
    [marques, search]
  );
  ...
  const deleteOne = async (m: MarqueRow) => {
    if (!confirm(`Supprimer "${m.nom}" ?`)) return;
    setError('');
    const res = await fetch(`/api/marques/${m.id}`, { method: "DELETE" });
    if (!res.ok) {
  ...
  const deleteSelected = async () => {
    ...
    if (!confirm(`Supprimer ${selectedIds.length} marque(s) ?`)) return;
    setError('');
    const results = await Promise.all(
      selectedIds.map(id => fetch(`/api/marques/${id}`, { method: "DELETE" }).then(r => r.json().then(d => ({ id, ok: r.ok, error: d.error }))))
    );
    const failed = results.filter(r => !r.ok);
    if (failed.length > 0) setError(failed.map(f => f.error).join(' · '));
    const deleted = results.filter(r => r.ok).map(r => r.id);
    setMarques(prev => prev.filter(x => !deleted.includes(x.id)));
    setSelectedIds([]);
  };
  ...
  return (
    <ErpPage
      title="Marques"
      subtitle={`${marques.length} marque${marques.length !== 1 ? 's' : ''}`}
      actions={
        <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
          ...
          <Link href="/erp/marques/nouvelle" style={{ fontSize: "0.75rem", color: "white", background: "#1A1208", textDecoration: "none", padding: "0.45rem 0.8rem", borderRadius: "3px" }}>
            Nouvelle marque
          </Link>
        </div>
      }
      ...
      <div style={{ background: "#FDFAF5", border: "1px solid #EDE5D4", borderRadius: "6px", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem", textAlign: "left" }}>
          <thead>
            <tr style={{ background: "#FAF7F2", borderBottom: "1px solid #EDE5D4" }}>
              <th style={{ padding: "0.75rem" }}>
                <input type="checkbox" checked={allSelected} onChange={e => setSelectedIds(e.target.checked ? filtered.map(m => m.id) : [])} />
              </th>
              <th style={{ padding: "0.75rem", fontWeight: 500, color: "#1A1208" }}>Nom</th>
              <th style={{ padding: "0.75rem", fontWeight: 500, color: "#1A1208" }}>Slug</th>
              <th style={{ padding: "0.75rem", fontWeight: 500, color: "#1A1208" }}>Description</th>
          ...
          <tbody>
            {paginated.length === 0 && (
              <tr>
                <td colSpan={6} style={{ padding: "2rem", textAlign: "center", color: "#C4B090", fontSize: "0.8rem" }}>
                  Aucune marque
                </td>
              </tr>
            )}
            {paginated.map(m => (
              <tr key={m.id} style={{ borderBottom: "1px solid #F0EBE0" }}>
                <td style={{ padding: "0.75rem" }}>
                  <input type="checkbox" checked={selectedIds.includes(m.id)} onChange={() => toggleSelection(m.id)} />
                </td>
                <td style={{ padding: "0.75rem", fontWeight: 500, color: "#1A1208" }}>{m.nom}</td>
                <td style={{ padding: "0.75rem", color: "#8A7B68", fontFamily: 'monospace', fontSize: '0.78rem' }}>{m.slug}</td>
                <td style={{ padding: "0.75rem", color: "#8A7B68", maxWidth: '240px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.description || '—'}</td>
                <td style={{ padding: "0.75rem", color: "#C4960A" }}>{m._count.produits}</td>
                <td style={{ padding: "0.75rem", display: "flex", gap: "0.5rem", alignItems: "center" }}>
                  <Link href={`/erp/marques/${m.id}`} style={{ color: "#C4960A", fontSize: "0.75rem", textDecoration: "none" }}>Modifier</Link>
                  <button onClick={() => deleteOne(m)} style={{ fontSize: "0.68rem", color: "#8B3A3A", border: "1px solid #FAEAEA", padding: "0.2rem 0.55rem", background: "#FAEAEA", cursor: "pointer", borderRadius: "3px" }}>
                    Supprimer
                  </button>
                </td>
              </tr>
