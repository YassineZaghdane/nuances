...
export default function ErpProduitsPage() {
  ...
  const allSelected = useMemo(
    () => produits.length > 0 && selectedIds.length === produits.length,
    [produits.length, selectedIds.length]
  );
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem", textAlign: "left" }}>
          <thead>
            <tr style={{ background: "#FAF7F2", borderBottom: "1px solid #EDE5D4" }}>
              <th style={{ padding: "0.75rem" }}><input type="checkbox" checked={allSelected} onChange={(e) => setSelectedIds(e.target.checked ? produits.map((p) => p.id) : [])} /></th>
