...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (e.code === 'P2025') return NextResponse.json({ error: "Marque introuvable" }, { status: 404 });
    throw e;
  }
}
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  ...
  const count = await prisma.produit.count({ where: { marqueId: params.id } });
  if (count > 0) {
    return NextResponse.json(
      { error: `Impossible : ${count} produit(s) utilisent cette marque` },
      { status: 409 }
    );
  }
