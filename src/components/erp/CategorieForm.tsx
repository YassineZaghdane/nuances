...
export function CategorieForm({ categorie, mode }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = mode === 'edition' && categorie?.id
      ...
      setSaved(true)
      setTimeout(() => { router.push('/erp/categories'); router.refresh() }, 900)
    } catch (err: any) {
    ...
    <div style={{ fontFamily: 'Jost,sans-serif', maxWidth: '520px' }}>
      ...
      {saved && (
        <div style={{ background: '#E4F2EB', border: '1px solid rgba(46,125,82,0.2)', color: '#2E7D52', padding: '0.8rem 1.2rem', fontSize: '0.82rem', marginBottom: '1rem', borderRadius: '3px' }}>
          ✓ Catégorie enregistrée — redirection…
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: '0.8rem' }}>
        <button
          onClick={() => router.push('/erp/categories')}
        ...
        >
          {saved ? '✓ Enregistré' : saving ? 'Enregistrement…' : mode === 'edition' ? 'Enregistrer' : 'Créer la catégorie'}
        </button>
      </div>
    </div>
  )
